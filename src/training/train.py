...
SEEDS = [42] 
BINS = 30 
EPOCHS = 25 # Reducido a 25
BATCH_SIZE = 64
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
OUTPUT_DIR = "results_cnn_main8_7refined"
...
# --- DATA AUGMENTATION (Torchvision Wrapper) ---
...
# --- MODELO CONVNET + BATCHNORM (Regularización en todo) ---
class ConvNetBN(nn.Module):
    def __init__(self, dropout_p=0.0, hidden_size=128, **kwargs):
        ...
        # Estructura: Conv -> BN -> ReLU -> Dropout2d -> Pool
        ...
        # Dropout Espacial (apaga feature maps completos)
        # Se aplica si p > 0
    ...
    def hidden_layer(self, x):
        ...
        x = self.bn1(x)
        x = F.relu(x)
        if self.p > 0: x = self.dropout2d(x)
        x = self.pool(x)
...
def train_network(net, reg_type, reg_val, train_loader, nepochs=EPOCHS, lr=0.01):
    ...
    # Scheduler: Reduce LR a la mitad en la época 15 para estabilizar y VER la mejora
    ...
    for it in range(nepochs):
        for x, t in train_loader:
            ...
            # REGULARIZACIÓN L1/L2 (Aplica a TODAS las capas con pesos)
...
def plot_sweep_results(df, title_suffix=''):
    ...
    # FIG 2: Accuracy (Eje único Train/Val)
    ...
    if SAVE_PLOTS: fig2.savefig(f'{OUTPUT_DIR}/accuracy_unified_{title_suffix}.png')
    plt.show()
...
def run_experiment(train_tensor, train_labels, title_suffix="FineTuning_Sweep"):
    # BARRIDO DE PRECISIÓN (Rango bajo y denso)
    # L1/L2: Nos enfocamos en el rango 1e-5 a 1e-3
    reg_vals = [1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3]
    
    # Dropout: Probabilidades más bajas para no romper la CNN
    drop_vals = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
    
    # Data Augmentation (Standard Torchvision)
    # Definimos transformaciones equivalentes
    t_none = None
    ...
    # Para plotear
    aug_levels_plot = [(l[0], l[1]) for l in aug_levels]
    
    param_lists = {'L1': reg_vals, 'L2': reg_vals, 'Dropout': drop_vals, 'DataAug': aug_levels_plot}
    ...
    # 3. DATA AUGMENTATION (Torchvision)
    method = 'DataAug'
    vals = aug_levels # Usamos la lista completa con transforms
    print(f"Training {method}...")
    for (aug_level, aug_name, transform) in vals:
        if aug_level == 0: continue
        for seed in SEEDS:
            torch.manual_seed(seed)
            # Dataset con Transform
