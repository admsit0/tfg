...
_DATASET_DEFAULT_BINS = {"CIFAR10": 30, "SVHN": 10}
N_BINS: int = 30  # overridden by --n-bins or dataset default in main()
...
def find_global_limits(device, base_models_dir, validation_loader, num_classes, best_models_info):
    print("Calculando limites globales del Baseline usando VALIDATION...")
    ...
    if not baseline_file:
        raise ValueError(f"No se encontro el .pth del baseline ({baseline_method} 0.0, ep {best_ep})")
    ...
    print(f"Limites calibrados: {limits}")
    return limits
...
def compute_unique_states(net, loader, device, layers, limits):
    ...
    with torch.no_grad():
        for x, _ in loader:
            ...
            for l in layers:
                ...
                min_val = limits[l]['min']
                max_val = limits[l]['max']
                
                act = torch.clamp(act, min=min_val, max=max_val)
                range_val = max_val - min_val + 1e-9
                
                bin_indices = torch.floor((act - min_val) / range_val * N_BINS).int()
                bin_indices = torch.clamp(bin_indices, 0, N_BINS - 1)
                
                for i in range(batch_size):
                    state = tuple(bin_indices[i].cpu().numpy())
    ...
    results_pct = {}
    for l in layers:
        results_pct[l] = (len(unique_states[l]) / total_images) * 100.0
        
    return results_pct, total_images
...
def generate_pdf_report(df, output_pdf, dataset_name):
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        fig.suptitle(f"EXECUTIVE SUMMARY: Internal States ORDERED ({dataset_name})\n(N_bins = {N_BINS}, Validation Set)", fontsize=20, fontweight='bold')
        ...
        for col_idx, layer in enumerate(layers):
            ...
            for m in methods:
                ...
                ax0.plot(sub['reg_val'], sub['unique_pctg_ORDERED'], marker='o', label=m, alpha=0.8)
                setup_log_scale_safe(ax0, m)
            ax0.set_title(f"{layer} - %_states_ORDERED vs Reg Val")
            ax0.set_ylabel("%_states_ORDERED")
            ...
            for m in methods:
                ...
                ax2.scatter(sub['val_acc'], sub['unique_pctg_ORDERED'], label=m, alpha=0.8, s=50)
            ax2.set_title(f"{layer} - %_states_ORDERED vs Validation Acc")
            ax2.set_xlabel("Val Acc")
            ax2.set_ylabel("%_states_ORDERED")
        ...
        for m in methods:
            ...
            for col_idx, layer in enumerate(layers):
                ...
                ax0 = fig.add_subplot(gs[0, col_idx])
                ax0.plot(df_layer_val['reg_val'], df_layer_val['unique_pctg_ORDERED'], marker='o', label='Val States', color='orange')
                ax0.plot(df_layer_train['reg_val'], df_layer_train['unique_pctg_ORDERED'], marker='x', label='Train States', color='blue', linestyle='--')
                setup_log_scale_safe(ax0, m)
                ax0.set_title(f"{layer} - %_states_ORDERED")
                ax0.set_ylabel("%_states_ORDERED")
                ax0.legend()
                ...
                ax2 = fig.add_subplot(gs[2, col_idx])
                ax2.scatter(df_layer_val['val_acc'], df_layer_val['unique_pctg_ORDERED'], label='Val', color='orange', s=60)
                ax2.scatter(df_layer_train['train_acc'], df_layer_train['unique_pctg_ORDERED'], label='Train', color='blue', marker='x', s=60)
                ax2.set_title(f"{layer} - %_states_ORDERED vs Acc")
...
def main(dataset_name):
    ...
    csv_output_path = os.path.join(output_base_dir, 'states_ORDERED_results.csv')
    pdf_output_path = os.path.join(output_base_dir, f'states_ORDERED_report_{dataset_name}.pdf')
    ...
    layers = ['conv1', 'conv3', 'fc1']
    global_limits = find_global_limits(device, models_dir, test_loader, num_classes, best_models_info)
    ...
    for (method, val), info in tqdm(best_models_info.items(), desc="Procesando modelos", colour='magenta'):
        if (method, val) not in pth_map:
            print(f"No se encontro el .pth para {method}={val} (Ep: {info['epoch']})")
            continue
        ...
        for l in layers:
            results.append({
                ...
                'layer': l, 'split': 'train', 
                'unique_pctg_ORDERED': train_states_pct[l],
                'num_images': n_train, 'n_bins': N_BINS
            })
            results.append({
                'reg_method': method, 'reg_val': val, 'best_epoch': info['epoch'],
                'val_acc': info['val_acc'], 'train_acc': info['train_acc'],
                'layer': l, 'split': 'val', 
                'unique_pctg_ORDERED': val_states_pct[l],
                'num_images': n_val, 'n_bins': N_BINS
            })
    ...
    print("Generando graficos y ensamblando PDF...")
    generate_pdf_report(df_results, pdf_output_path, dataset_name)
    print(f"Reporte PDF completado: {pdf_output_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ordered-state unique_pctg analysis over trained CNN checkpoints.")
    parser.add_argument('--dataset', type=str, required=True,
                        choices=['CIFAR10', 'SVHN', 'FashionMNIST', 'CIFAR100'])
    parser.add_argument('--n-bins', type=int, default=None,
                        help="Number of discretization bins per neuron. Default: 30 for CIFAR10, 10 for SVHN.")
    args = parser.parse_args()

    global N_BINS
    N_BINS = args.n_bins if args.n_bins is not None else _DATASET_DEFAULT_BINS.get(args.dataset, 30)
    print(f"[config] dataset={args.dataset}  n_bins={N_BINS}")
