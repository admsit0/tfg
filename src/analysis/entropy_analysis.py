...
N_BINS: int = 10  # overridden by --n-bins or dataset default in main()
...
def compute_unique_states(net, loader, device, layers, limits):
    ...
    entropy_sum = {l: 0.0 for l in layers}
    total_images = 0
    ...
    for l in layers:
        ...
        results_ent[l] = entropy_sum[l] / total_images
        
    return results_pct, results_ent, total_images
...
def generate_pdf_report(df, output_pdf, dataset_name):
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        for col_idx, layer in enumerate(layers):
            ...
            for m in methods:
                sub = df_layer[df_layer['reg_method'] == m].sort_values('reg_val')
                ax1.plot(sub['reg_val'], sub['entropy'], marker='s', linestyle='--', label=m, alpha=0.8)
                setup_log_scale_safe(ax1, m)
            ax1.set_title(f"{layer} - Entropy vs Reg Val")
        ...
        for m in methods:
            ...
            for col_idx, layer in enumerate(layers):
                ...
                ax1 = fig.add_subplot(gs[1, col_idx])
                ax1.plot(df_layer_val['reg_val'], df_layer_val['entropy'], marker='o', label='Val Entropy', color='orange')
                ax1.plot(df_layer_train['reg_val'], df_layer_train['entropy'], marker='x', label='Train Entropy', color='blue', linestyle='--')
                setup_log_scale_safe(ax1, m)
                ax1.set_title("Activation Entropy")
                ax1.set_ylabel("Entropy (bits)")
                ax1.legend()
...
def main(dataset_name):
    ...
    for (method, val), info in tqdm(best_models_info.items(), desc="Procesando modelos", colour='magenta'):
        ...
        train_states_pct, train_entropy, n_train = compute_unique_states(net, train_loader, device, layers, global_limits)
        val_states_pct, val_entropy, n_val = compute_unique_states(net, test_loader, device, layers, global_limits)
        ...
        for l in layers:
            results.append({
                ...
                'layer': l, 'split': 'train', 
                'unique_pctg': train_states_pct[l], 'entropy': train_entropy[l],
            ...
            results.append({
                ...
                'val_acc': info['val_acc'], 'train_acc': info['train_acc'],
                'layer': l, 'split': 'val', 
                'unique_pctg': val_states_pct[l], 'entropy': val_entropy[l],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Entropy & unique-state analysis over trained CNN checkpoints.")
    parser.add_argument('--dataset', type=str, required=True,
                        choices=['CIFAR10', 'SVHN', 'FashionMNIST', 'CIFAR100'])
    parser.add_argument('--n-bins', type=int, default=None,
                        help="Number of histogram bins. Default: 30 for CIFAR10, 10 for SVHN.")
    args = parser.parse_args()

    N_BINS = args.n_bins if args.n_bins is not None else _DATASET_DEFAULT_BINS.get(args.dataset, 10)
