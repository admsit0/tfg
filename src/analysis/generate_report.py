...
def generate_block(pdf, title, base_path, methods, global_vmin, global_vmax):
    ...
    # 1. Extraer BASELINE dinámicamente (reg_val == 0.0)
    baseline_added = False
    for fallback_method in ["L1", "L2", "Dropout", "DataAug"]:
        if fallback_method in dfs:
            df_m = dfs[fallback_method]
            df_base = df_m[df_m['reg_val'] == 0.0]
            if not df_base.empty:
                best_idx = df_base['val_acc'].idxmax()
                best_row = df_base.loc[best_idx]
                summary_rows.append({
                ...
                baseline_added = True
                break # Solo extraemos el baseline una vez

    # 2. Computar el resumen del resto de métodos
    for m in methods:
        if m in dfs:
    ...
    # 3. Ordenar la tabla: Baseline primero, el resto ordenado por Mejor Accuracy
    if baseline_added:
        df_baseline = summary_df[summary_df["Method"] == "Baseline (No Reg)"]
        df_others = summary_df[summary_df["Method"] != "Baseline (No Reg)"].sort_values(by="Best Val Acc", ascending=False)
        summary_df = pd.concat([df_baseline, df_others], ignore_index=True)
    else:
    ...
    # Estilizar encabezados de tabla y destacar el Baseline
    for (row, col), cell in table.get_celld().items():
        if row == 0:
            cell.set_text_props(weight='bold', color='white')
            cell.set_facecolor('#40466e')
        elif summary_display.iloc[row-1]["Method"] == "Baseline (No Reg)":
            cell.set_facecolor('#ffe6e6') # Fondo distintivo para el baseline
            cell.set_text_props(style='italic', weight='bold')
        elif row % 2 == 0:
...
# MAIN
# =========================================================
def main(dir_main, output_pdf, compare_mode=False, dir_compare=None):
    print(f"🔍 Escaneando carpeta en busca de resultados en: {dir_main}")
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        if compare_mode and dir_compare:
            print(f"🔀 Generando sección de comparativa cruzada con {dir_compare}...")
            generate_cross_comparison(pdf, dir_main, dir_compare, methods)

    print(f"🚀 ¡PDF generado exitosamente en: {output_pdf}!")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generador de Reportes de Regularización Multidataset")
    
    # El usuario pasa el dataset (ej: CIFAR10) y el script construye la ruta
    parser.add_argument('--dataset', type=str, required=True, help="Nombre del dataset (ej. CIFAR10, SVHN, FashionMNIST)")
    
    # Opcional: Si quieres comparar con otro dataset o carpeta
    parser.add_argument('--compare_dataset', type=str, default=None, help="Nombre de otro dataset para comparar (ej. CIFAR10_noBN)")
    
    args = parser.parse_args()
    
    # Construcción dinámica de rutas (sin constantes arriba)
    dir_main = f"outputs_{args.dataset}/data"
    output_pdf = f"outputs_{args.dataset}/comprehensive_report_{args.dataset}.pdf"
    
    compare_mode = args.compare_dataset is not None
    dir_compare = f"outputs_{args.compare_dataset}/data" if compare_mode else None
    
    main(dir_main, output_pdf, compare_mode, dir_compare)