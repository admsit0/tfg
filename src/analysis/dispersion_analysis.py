...
def compute_dispersion(net, loader, device, layers):
    ...
    with torch.no_grad():
        for x, _ in loader:
            ...
            for l in layers:
                ...
                all_acts[l].append(act.cpu())
                
    for h in handles: h.remove()
    
    stds = {}
    for l in layers:
        cat_acts = torch.cat(all_acts[l], dim=0)
        stds[l] = cat_acts.std().item()
        
    return stds, total_images

def get_baseline_stds(device, base_models_dir, train_loader, test_loader, num_classes, best_models_info):
    ...
    train_stds, _ = compute_dispersion(net, train_loader, device, layers)
    val_stds, _ = compute_dispersion(net, test_loader, device, layers)
    
    return train_stds, val_stds
...
def generate_pdf_report(df, output_pdf, dataset_name):
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        fig.suptitle(f"EXECUTIVE SUMMARY: Dispersion ({dataset_name})\nValidation Set", fontsize=20, fontweight='bold')
        ...
        for col_idx, layer in enumerate(layers):
            ...
            for m in methods:
                ...
                ax0.plot(sub['reg_val'], sub['dispersion_ratio'], marker='o', label=m, alpha=0.8)
                setup_log_scale_safe(ax0, m)
            ax0.set_title(f"{layer} - Dispersion Ratio vs Reg Val")
            ax0.set_ylabel("Dispersion Ratio")
            ...
            for m in methods:
                ...
                ax2.scatter(sub['val_acc'], sub['dispersion_ratio'], label=m, alpha=0.8, s=50)
            ax2.set_title(f"{layer} - Dispersion Ratio vs Validation Acc")
            ax2.set_xlabel("Val Acc")
            ax2.set_ylabel("Dispersion Ratio")
        ...
        for m in methods:
            ...
            for col_idx, layer in enumerate(layers):
                ...
                ax0 = fig.add_subplot(gs[0, col_idx])
                ax0.plot(df_layer_val['reg_val'], df_layer_val['dispersion_ratio'], marker='o', label='Val Ratio', color='orange')
                ax0.plot(df_layer_train['reg_val'], df_layer_train['dispersion_ratio'], marker='x', label='Train Ratio', color='blue', linestyle='--')
                setup_log_scale_safe(ax0, m)
                ax0.set_title(f"{layer} - Dispersion Ratio")
                ax0.set_ylabel("Dispersion Ratio")
                ax0.legend()
                ...
                ax2 = fig.add_subplot(gs[2, col_idx])
                ax2.scatter(df_layer_val['val_acc'], df_layer_val['dispersion_ratio'], label='Val', color='orange', s=60)
                ax2.scatter(df_layer_train['train_acc'], df_layer_train['dispersion_ratio'], label='Train', color='blue', marker='x', s=60)
                ax2.set_title(f"{layer} - Dispersion Ratio vs Acc")
...
def main(dataset_name):
    ...
    csv_output_path = os.path.join(output_base_dir, 'dispersion_ratio_results.csv')
    pdf_output_path = os.path.join(output_base_dir, f'dispersion_ratio_report_{dataset_name}.pdf')
    ...
    layers = ['conv1', 'conv3', 'fc1']
    baseline_train_stds, baseline_val_stds = get_baseline_stds(device, models_dir, train_loader, test_loader, num_classes, best_models_info)
    ...
    for (method, val), info in tqdm(best_models_info.items(), desc="Procesando modelos", colour='magenta'):
        ...
        train_stds, n_train = compute_dispersion(net, train_loader, device, layers)
        val_stds, n_val = compute_dispersion(net, test_loader, device, layers)
        ...
        for l in layers:
            results.append({
                ...
                'layer': l, 'split': 'train', 
                'dispersion_ratio': train_stds[l] / max(baseline_train_stds[l], 1e-9),
                'num_images': n_train
            })
            results.append({
                'reg_method': method, 'reg_val': val, 'best_epoch': info['epoch'],
                'val_acc': info['val_acc'], 'train_acc': info['train_acc'],
                'layer': l, 'split': 'val', 
                'dispersion_ratio': val_stds[l] / max(baseline_val_stds[l], 1e-9),
                'num_images': n_val
            })
