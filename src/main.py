...
def train_and_evaluate_loaders(cfg, reg_combo_name: str, reg_cfgs: list, run_dir: Path, train_loader, val_loader, reg_objs, pbar: Optional[tqdm] = None):
    """
    Trains and evaluates a model for a configuration, using provided DataLoaders.
    This function contains the refactored training/evaluation logic.
    """
    
    # --- 1. Build Model and Optimizer ---
    # Build the model INSIDE the function to ensure it's reset for each fold/run
    model_cfg = cfg.get('model', {})
    model_name = model_cfg.get('name', 'simple_cnn')
    model_kwargs = model_cfg.get('kwargs', {})
    model = build_model(model_name, dataset=cfg.get('data', {}).get('dataset', 'cifar10'), regularizers=reg_cfgs, **model_kwargs)
    
    # Apply regularizers (like Dropout) to the model
    for ro in reg_objs:
        try:
            model = ro.apply_to_model(model)
        except Exception:
            pass # Not all regularizers have apply_to_model
    ...
    # --- 2. Prepare Logging and Analysis ---
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            # Add regularization penalties (L1, L2)
            ...
            total_loss = loss + reg_pen # Renamed from 'loss' to 'total_loss' for clarity
            total_loss.backward()
            
            # --- HERE'S YOUR SOLUTION! ---
            # Apply gradient clipping to stabilize L1
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            
            optimizer.step()

            train_loss += loss.item() * yb.size(0) # Log only the 'loss' (cross-entropy)
        ...
        val_total = 0
        with torch.no_grad():
            for xb, yb in val_loader: # Use val_loader (passed as argument)
                xb = xb.to(device)
                yb = yb.to(device)
                logits = model(xb)
                if isinstance(logits, tuple):
        ...
        if pbar is not None:
            ...
            # Print only if pbar is not active (to avoid duplicate logs)
            print(f"    Epoch {epoch}/{max_epochs}: train_acc={train_acc:.4f} val_acc={val_acc:.4f}")

        # Save metrics from the last epoch
        if epoch == max_epochs:
            final_metrics = {
        ...
        # BUG FIXED: This 'if' was improperly indented in the original code
        if should_collect and collect_layer:
            try:
                # Collect activations on the provided 'val_loader'
                feats, trues, preds = collect_activations(model, val_loader, device, collect_layer)
                if feats.size == 0:
                ...
                if save_per_epoch:
                    # 'run' is for the non-cv case, 'foldX' is handled in the CV part
                    run_suffix = run_dir.name if run_dir.name.startswith('fold') else 'run'
                    out_name = f"{stem}_epoch{epoch}_{run_suffix}{suffix}"
                else:
                    # For CV, 'run_dir' is already the 'fold_dir', so acts_fname is fine
                    # For non-CV, this saves to 'run/activations.csv'
                    out_name = acts_fname 
                ...
                # Cross-fold CV collection logic (saving activations from other folds)
                # This logic was moved to 'expand_regularizers_and_run'
    ...
    # End of epoch loop
    csv_logger.close()
    # pbar is closed in the function that creates it
    
    # Return the trained model and final metrics
...
def expand_regularizers_and_run(cfg):
    ...
    for combo_name, regs in combinations:
        ...
        if cv_enabled and n_folds > 1:
            ...
            # build dataset but return the underlying dataset object
            ds_cfg = cfg.get('data', {})
            batch_size = ds_cfg.get('batch_size', 128)
            ...
            fold_results = []
            trained_models_per_fold = {} # Save models for cross-collection
            ...
            for fold_idx, (train_idx, val_idx) in enumerate(folds, start=1):
                ...
                # 1. Prepare DataLoaders for this fold
                ...
                # 2. Prepare Seed and Regularizers for this fold
                import hashlib
                ...
                # 3. Prepare pbar (if enabled)
                if tqdm_enabled:
                    max_epochs = int(cfg.get('trainer', {}).get('max_epochs', 10))
                    pbar = tqdm(total=max_epochs, desc=f"{combo_name} fold{fold_idx}")
                else:
                    pbar = None

                # 4. Call the refactored training function
                try:
                    # 'regs' here is the 'reg_cfgs' expected by the function
                    trained_model, metrics = train_and_evaluate_loaders(
                        cfg, combo_name, regs, fold_dir,
                        train_loader, val_loader, # Pass val_loader as the validation loader
                        reg_objs, pbar
                    )
                    
                    trained_models_per_fold[fold_idx] = trained_model
            ...
            # --- 5. (Optional) Cross-Activation Collection ---
            # This logic must remain here, as it needs the models from *all* folds
            analysis = cfg.get('analysis', {})
            collect_layer = analysis.get('layers', None)
            ...
            if save_training and collect_layer:
                print("  Running cross-fold activation collection...")
                for model_fold_idx, model in trained_models_per_fold.items():
                    # Output directory for this model
                    model_fold_dir = run_dir / f"fold{model_fold_idx}"
                    
                    # Iterate over validation data from *other* folds
                    for data_fold_idx, (t_idxs, v_idxs) in enumerate(folds, start=1):
                        if data_fold_idx == model_fold_idx:
                            continue # Already saved this during training
                        
                        try:
                            # Create a loader for the validation set of this data_fold
                            val_subset = torch.utils.data.Subset(train_dataset, v_idxs)
                            def worker_init_fn_generic(worker_id, base= (int(cv_seed) + int(data_fold_idx)) % (2 ** 31 - 1)):
                                seed = (int(base) + worker_id) % (2 ** 31 - 1)
                                import random as _random, numpy as _np, torch as _torch
                                _random.seed(seed)
                                _np.random.seed(seed)
                                try: _torch.manual_seed(seed)
                                except Exception: pass

                            val_loader_other = torch.utils.data.DataLoader(
                                val_subset, batch_size=batch_size, shuffle=False, worker_init_fn=worker_init_fn_generic
                            )
                        
                            feats, trues, preds = collect_activations(model, val_loader_other, device, collect_layer)
                            if feats.size == 0: combined = np.zeros((0, 0))
                            else: combined = np.concatenate([feats, preds.reshape(-1, 1), trues.reshape(-1, 1)], axis=1)

                            # Save to the *model's* directory (model_fold_dir)
                            out_name = f"{stem}_fold{data_fold_idx}{suffix}"
                            out_path = model_fold_dir / out_name
                            
                            if combined.size == 0:
                                header = 'pred_label,true_label'
                                with open(out_path, 'w', newline='') as fh: fh.write(header + '\n')
                            else:
                                ncols = combined.shape[1]
                                act_cols = [f"act_{i}" for i in range(ncols - 2)]
                                header = ','.join(act_cols + ['pred_label', 'true_label'])
                            ...
                            print(f"Warning: failed cross-collect activations for model {model_fold_idx} on data {data_fold_idx}: {e}")

            # --- End of Fold Loop ---
            print(f"Averaging results for combo {combo_name} over {len(fold_results)} folds")
            # (CSV aggregation logic would go here)

        else:
            # --- Single Execution (No-CV) ---
            try:
                ...
                # 3. Prepare Directory and pbar
                fold_dir = run_dir / 'run' # 'run' directory for single execution
                fold_dir.mkdir(parents=True, exist_ok=True)
                
                if tqdm_enabled:
                    max_epochs = int(cfg.get('trainer', {}).get('max_epochs', 10))
                    pbar = tqdm(total=max_epochs, desc=combo_name)
                else:
                    pbar = None

                # 4. Call the refactored training function
                # 'regs' is the 'reg_cfgs' expected by the function
                _, metrics = train_and_evaluate_loaders(
                    cfg, combo_name, regs, fold_dir,
                    train_loader, test_loader, # Pass test_loader as val_loader
                    reg_objs, pbar
                )
                ...
                # 'train_and_evaluate_loaders' already prints the final message
            
            except Exception as e:
                print(f"Experiment {combo_name} failed: {e}")
                # Make sure to close pbar if it fails
                if tqdm_enabled and 'pbar' in locals() and pbar is not None:
                    pbar.close()
