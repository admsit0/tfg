...
CONFIG = {
    'EPOCHS': 60,           # Épocas altas para convergencia fina
    'BATCH_SIZE': 64,
    'SEEDS': [42],
    'DEVICE': 'cuda' if torch.cuda.is_available() else 'cpu',
    'OUTPUT_DIR': os.path.join(SCRIPT_DIR, 'outputs-higherGran'),
    'DATA_LIMIT': 25000
}
...
# --- 3. DEFINICIÓN DE LA RED (CNN CON BATCHNORM) ---
class ConvNet(nn.Module):
    def __init__(self, dropout_p=0.0):
        super(ConvNet, self).__init__()
        self.dropout_p = dropout_p
        
        # CON BATCHNORM (Restaurado)
    ...
    def forward(self, x):
        # Bloques CON BN: Conv -> BN -> ReLU -> Dropout -> Pool
...
def train_experiment(net_name, reg_method, reg_val, train_loader, test_loader):
    ...
    pbar_epoch = tqdm(range(1, CONFIG['EPOCHS'] + 1), desc="Progreso", unit="epoch", leave=True, colour='green')
    ...
    for epoch in pbar_epoch:
        ...
        pbar_epoch.set_postfix({'Train': f'{train_acc:.3f}', 'Val': f'{val_acc:.3f}'})
...
# --- 5. MACROBUCLE PRINCIPAL ---
def run_macro_experiment():
    print("""
    ===================================================
      INICIANDO EXPERIMENTO "OPCIÓN A" (CON BATCHNORM)
    ===================================================
    """)
    setup_directories()
    ...
    for net_name in NET_TYPES:
        ...
        for reg_method, param_values in EXPERIMENTS.items():
            ...
            print(f"✅ Finalizado bloque {reg_method}. CSV guardado.")
