...
# --- FUNCIONES DE ANÁLISIS DE ESTADOS (MODIFICADO: Lógica Original Notebook) ---
...
def compute_simple_bin_edges(activations, num_bins=BINS):
    ...
    # Usamos float para evitar errores de tipo si vienes de enteros
...
# --- PLOTTING (MODIFICADO: Eje Y en Porcentaje) ---
def plot_sweep_results(df, title_suffix=''):
    if df.empty: return

    baseline_df = df[df['reg_type'] == 'Baseline']
    # Medias de Baseline (ya vienen en % en el dataframe)
    ...
    # FIG 1: Internal States % (Train vs Val)
    ...
    for i, method in enumerate(methods):
        ...
        # Agrupar para obtener media y std
        ...
        ax.set_ylim(-5, 105) # Fijar eje Y de 0 a 100% aprox
        ax.legend()
...
def run_experiment(train_dataset_exp, title_suffix="FullData"):
    # Extraemos el tensor de entrenamiento real usado en este dataset
    # Esto es crucial para que el % sea correcto en el dataset reducido (sobre 1000 muestras, no 60000)
    ...
    print("Training Baseline & Computing SIMPLE Bin Edges...")
    bin_edges = None
    
    # Número total de muestras para calcular porcentajes
    ...
    for seed in SEEDS:
        ...
        # 1. Calcular Bins SIMPLES sobre Baseline
        if bin_edges is None:
            net.eval()
            with torch.no_grad():
                # Importante: calculamos bordes usando el tensor de entrenamiento global
                # para capturar el rango dinámico completo posible
